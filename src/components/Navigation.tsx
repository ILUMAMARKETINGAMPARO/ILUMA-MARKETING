...
const Navigation = () => {
  ...
  return (
    <nav className="sticky top-0 left-0 right-0 z-[100] bg-black/80 backdrop-blur-md shadow-2xl border-b border-white/20" role="navigation" aria-label="Menu principal">
      <div className="flex items-center justify-between h-16 px-6 max-w-7xl mx-auto">
              ...
              <div key={item.name} className="relative">
                {item.hasDropdown ? (
                  <button 
                    ...
                    className={`flex items-center space-x-2 text-sm font-medium transition-all duration-200 hover:scale-105 font-['Montserrat'] px-3 py-2 rounded-lg ${isActive ? 'text-[#FFD56B] bg-[#FFD56B]/10' : 'text-white/70 hover:text-white/100 hover:bg-white/5'}`}
                  >
                    <span>{item.name}</span>
                  </button>
                ) : (
                  <Link 
                    to={item.path} 
                    className="text-sm font-medium text-white/70 hover:text-white/100 transition-all duration-200 hover:scale-105 font-['Montserrat'] px-3 py-2 rounded-lg hover:bg-white/5" 
        ...
        <div className="flex items-center space-x-4">
          {/* Language Selector - Desktop uniquement */}
          ...
          <Button 
            ...
            className="hidden lg:block bg-gradient-to-r from-orange-500 to-red-500 hover:from-red-500 hover:to-orange-500 text-white font-semibold px-3 py-2 rounded-full text-xs transition-all duration-200 hover:scale-105 hover:shadow-lg hover:shadow-orange-500/25 font-['Montserrat'] group relative overflow-hidden"
          >
          ...
          <Link to="/contact" onClick={closeDropdowns}>
            <Button size="sm" className="hidden lg:block bg-gradient-to-r from-[#FFD56B] to-[#8E44FF] hover:from-[#8E44FF] hover:to-[#FFD56B] text-black font-semibold px-4 py-2 rounded-full text-xs transition-all duration-200 hover:scale-105 hover:shadow-lg hover:shadow-[#FFD56B]/25 font-['Montserrat'] group relative overflow-hidden">
      ...
      {/* Mobile Navigation */}
      {isOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 mt-2 bg-black/90 backdrop-blur-xl rounded-2xl border border-white/10 p-4">
